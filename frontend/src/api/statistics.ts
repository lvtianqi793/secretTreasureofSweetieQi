/**
 * 能耗统计图表 API（使用环境变量配置后端地址，支持开发和生产环境）
 */
...
/**
 * 构建完整的API URL
 * @param path API路径（如 '/api/energy/options'）
 * @returns 完整的URL
 */
function buildApiUrl(path: string): string {
  // 如果有配置基础URL则使用，否则使用相对路径（依赖Vite代理或同域部署）
...
export type EnergyOptionsPayload = {
...
export async function postStatisticsExport(body: StatisticsExportBody) {
  const url = buildApiUrl("/api/statistics/export");
...
export async function postChartExport(body: ChartRequest) {
  const url = buildApiUrl("/api/statistics/chart/export");
  const res = await fetch(url, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(body),
...
export async function postEnergyQueryExport(body: EnergyQueryExportBody) {
  ...
  const baseUrl = buildApiUrl("/api/energy/query/export");
