...
-- 10. 气象数据表 (站点编号, 监测时间, 环境温度, 湿度, 风速)
CREATE TABLE IF NOT EXISTS weather_data (
    id BIGSERIAL PRIMARY KEY,
    building_id VARCHAR(100) NOT NULL,           -- 站点编号
    building_type VARCHAR(50) NOT NULL,          -- 固定值 'weather_station'
    monitor_time TIMESTAMP NOT NULL,             -- 监测时间
    temperature_c DOUBLE PRECISION,              -- 环境温度 (°C)
    humidity_pct DOUBLE PRECISION,               -- 湿度 (%RH)
    wind_speed_ms DOUBLE PRECISION               -- 风速 (m/s)
);
