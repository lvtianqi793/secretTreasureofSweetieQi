...
import com.energy.management.entity.GasData;
...
public interface GasDataRepository extends JpaRepository<GasData, Long> {
    ...
    @Query("SELECT e FROM GasData e WHERE " +
    ...
    Page<GasData> queryByConditions(
    ...
    List<GasData> findByBuildingIdAndMonitorTimeBetween(
            String buildingId, LocalDateTime start, LocalDateTime end);

    @Query("SELECT DISTINCT e.buildingId FROM GasData e")
