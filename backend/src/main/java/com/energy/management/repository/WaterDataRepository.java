...
import com.energy.management.entity.WaterData;
...
public interface WaterDataRepository extends JpaRepository<WaterData, Long> {
    ...
    @Query("SELECT e FROM WaterData e WHERE " +
    ...
    Page<WaterData> queryByConditions(
    ...
    List<WaterData> findByBuildingIdAndMonitorTimeBetween(
            String buildingId, LocalDateTime start, LocalDateTime end);

    @Query("SELECT DISTINCT e.buildingId FROM WaterData e")
